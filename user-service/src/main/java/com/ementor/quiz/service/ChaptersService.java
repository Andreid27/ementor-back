...
public class ChaptersService {
	...
	public List<FilterOption<?>> filterOptions(PaginatedRequest request) {
		return FilterOptionUtils.createFilterOptions(entityManager, request, Chapter.class,
				new FilterGroup("title", FilterType.TEXT_OPTIONS), new FilterGroup("description", FilterType.TEXT_CONTENT));
	}
	...
	public List<Chapter> getChaptersByIds(List<UUID> chaptersIds) {
		return chaptersRepo.findAllByIdIn(chaptersIds);
	}
}
